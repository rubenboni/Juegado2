...
void UFloorPlanBuilder::BuildDungeonImpl(UWorld* World) {
    FloorPlanModel = Cast<UFloorPlanModel>(DungeonModel);
    FloorPlanConfig = Cast<UFloorPlanConfig>(DungeonConfig);
...
void UFloorPlanBuilder::BuildLayout() {
    ...
    for (int z = 0; z < NumFloors; z++) {
        ...
        for (FloorChunkPtr Chunk : FloorChunks) {
            TArray<FloorChunkPtr> Stack;
            Stack.Push(Chunk);
            const int32 MinRoomSize = FMath::Max(FloorPlanConfig->MinRoomSize, FloorPlanConfig->MaxRoomSize);
...
FString GetDoorMarkerName(FloorChunkPtr ChunkA, FloorChunkPtr ChunkB) {
    if (!ChunkA.IsValid() && !ChunkB.IsValid()) {
        return FFloorPlanBuilderMarkers::MARKER_DOOR;
    }
    ...
    return PreferedChunk->DoorMarker.Len() > 0 ? PreferedChunk->DoorMarker : FFloorPlanBuilderMarkers::MARKER_DOOR;
}
...
void UFloorPlanBuilder::EmitDungeonMarkers_Implementation() {
    ...
    for (int z = 0; z < NumFloors; z++) {
        for (int x = -1; x < FloorPlanConfig->BuildingSize.X; x++) {
            ...
            for (int y = -1; y < FloorPlanConfig->BuildingSize.Y; y++) {
                ...
                FString GroundMarkerName = FFloorPlanBuilderMarkers::MARKER_GROUND;
                FString CeilingMarkerName = FFloorPlanBuilderMarkers::MARKER_CEILING;
                ...
                */
                ...
                FString DoorMarkerName = FFloorPlanBuilderMarkers::MARKER_DOOR;
                ...
                if (bEmitLeftWall) {
                    ...
                    if (bLeftDoor) {
                        ...
                        MarkerName = FFloorPlanBuilderMarkers::MARKER_WALL;
                        if (bBuildingWallLeft) {
                            MarkerName = FFloorPlanBuilderMarkers::MARKER_BUILDING_WALL;
                        }
                        else {
    ...
    EmitSeparatorMarkers(FFloorPlanBuilderMarkers::MARKER_WALL, FFloorPlanBuilderMarkers::MARKER_WALL_SEPARATOR, GridSize);
    EmitSeparatorMarkers(FFloorPlanBuilderMarkers::MARKER_BUILDING_WALL, FFloorPlanBuilderMarkers::MARKER_BUILDING_WALL_SEPARATOR, GridSize);
}
...
void UFloorPlanBuilder::GetDefaultMarkerNames(TArray<FString>& OutMarkerNames) {
    ...
    OutMarkerNames.Add(FFloorPlanBuilderMarkers::MARKER_GROUND);
    OutMarkerNames.Add(FFloorPlanBuilderMarkers::MARKER_CEILING);
    OutMarkerNames.Add(FFloorPlanBuilderMarkers::MARKER_WALL);
    OutMarkerNames.Add(FFloorPlanBuilderMarkers::MARKER_WALL_SEPARATOR);
    OutMarkerNames.Add(FFloorPlanBuilderMarkers::MARKER_DOOR);
    OutMarkerNames.Add(FFloorPlanBuilderMarkers::MARKER_BUILDING_WALL);
    OutMarkerNames.Add(FFloorPlanBuilderMarkers::MARKER_BUILDING_WALL_SEPARATOR);
}
...
TSharedPtr<IMarkerGenProcessor> UFloorPlanBuilder::CreateMarkerGenProcessor(const FTransform& InDungeonTransform) const {
    if (const UFloorPlanConfig* Config = FloorPlanConfig ? FloorPlanConfig : Cast<UFloorPlanConfig>(DungeonConfig)) {
