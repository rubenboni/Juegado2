...
void USimpleCityBuilder::EmitDungeonMarkers_Implementation() {
    ...
    for (int x = 0; x < width; x++) {
        for (int y = 0; y < length; y++) {
            ...
            if (Cell.CellType == ESimpleCityCellType::House) {
                MarkerName = FSimpleCityBuilderMarkers::MarkerHouse;
                Rotation = Cell.Rotation;
            }
            if (Cell.CellType == ESimpleCityCellType::Park) {
                MarkerName = FSimpleCityBuilderMarkers::MarkerPark;
                Rotation = Cell.Rotation;
            }
...
void USimpleCityBuilder::GetDefaultMarkerNames(TArray<FString>& OutMarkerNames) {
    ...
    OutMarkerNames.Add(FSimpleCityBuilderMarkers::MarkerHouse);
    OutMarkerNames.Add(FSimpleCityBuilderMarkers::MarkerPark);
    OutMarkerNames.Add(FSimpleCityBuilderMarkers::MarkerRoadX);
    OutMarkerNames.Add(FSimpleCityBuilderMarkers::MarkerRoadT);
    OutMarkerNames.Add(FSimpleCityBuilderMarkers::MarkerRoadS);
    OutMarkerNames.Add(FSimpleCityBuilderMarkers::MarkerRoadCorner);
    OutMarkerNames.Add(FSimpleCityBuilderMarkers::MarkerRoadEnd);
}

TSharedPtr<IMarkerGenProcessor> USimpleCityBuilder::CreateMarkerGenProcessor(const FTransform& InDungeonTransform) const {
    if (const USimpleCityConfig* Config = CityConfig ? CityConfig : Cast<USimpleCityConfig>(DungeonConfig)) {
