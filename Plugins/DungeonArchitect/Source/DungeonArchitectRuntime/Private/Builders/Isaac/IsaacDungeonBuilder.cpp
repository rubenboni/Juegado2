...
void UIsaacDungeonBuilder::BuildDungeonImpl(UWorld* World) {
    isaacModel = Cast<UIsaacDungeonModel>(DungeonModel);
    isaacConfig = Cast<UIsaacDungeonConfig>(DungeonConfig);
        ...
        for (int x = -1; x < roomWidth + 1; x++) {
            for (int y = -1; y < roomHeight + 1; y++) {
                ...
                if (tile.tileType == EIsaacRoomTileType::Floor) {
                    CreateMarkerAt(x + 0.5f, y + 0.5f, 0, FIsaacBuilderMarkers::ST_GROUND, roomBasePosition, tileSize);
                }

            }
        }

        EmitBoundaryMarkers(FIsaacBuilderMarkers::ST_WALL, EIsaacRoomTileType::Empty, EIsaacRoomTileType::Floor, room,
                            roomWidth, roomHeight, roomBasePosition, tileSize);
        EmitBoundaryMarkers(FIsaacBuilderMarkers::ST_DOOR, EIsaacRoomTileType::Door, EIsaacRoomTileType::Floor, room,
                            roomWidth, roomHeight, roomBasePosition, tileSize);
    }
}
...
void UIsaacDungeonBuilder::GetDefaultMarkerNames(TArray<FString>& OutMarkerNames) {
    ...
    OutMarkerNames.Add(FIsaacBuilderMarkers::ST_GROUND);
    OutMarkerNames.Add(FIsaacBuilderMarkers::ST_WALL);
    OutMarkerNames.Add(FIsaacBuilderMarkers::ST_DOOR);
}
