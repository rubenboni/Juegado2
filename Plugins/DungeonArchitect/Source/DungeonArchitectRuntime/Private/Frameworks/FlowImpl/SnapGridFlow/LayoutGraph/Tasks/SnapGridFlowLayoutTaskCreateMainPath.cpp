...
DEFINE_LOG_CATEGORY_STATIC(LogSGFCreateMainPath, Log, All);

IFlowAGNodeGroupGeneratorPtr USnapGridFlowLayoutTaskCreateMainPath::CreateNodeGroupGenerator(TWeakPtr<const IFlowDomain> InDomainPtr) const {
...
FFlowAbstractGraphConstraintsPtr USnapGridFlowLayoutTaskCreateMainPath::CreateGraphConstraints(TWeakPtr<const IFlowDomain> InDomainPtr) const {
    const TSharedPtr<const IFlowDomain> Domain = InDomainPtr.Pin();
    if (!Domain.IsValid() || Domain->GetDomainID() != FSnapGridFlowAbstractGraphDomain::DomainID) {
...
UFlowLayoutNodeCreationConstraint* USnapGridFlowLayoutTaskCreateMainPath::GetNodeCreationConstraintLogic() const {
    return bUseNodeCreationConstraint ? NodeCreationConstraint : nullptr;
}

void USnapGridFlowLayoutTaskCreateMainPath::FinalizePath(const FFlowAGStaticGrowthState& StaticState, FFlowAGGrowthState& State) const {
