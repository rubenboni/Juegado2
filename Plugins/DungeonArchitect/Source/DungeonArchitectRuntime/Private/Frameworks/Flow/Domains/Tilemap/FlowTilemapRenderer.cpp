...
DEFINE_LOG_CATEGORY_STATIC(LogFlowTileRenderer, Log, All);

class FFlowCanvasHelper {
public:
...
namespace {
    FLinearColor GetEdgeColor(const FFlowTilemapEdge& InEdge) {
        switch(InEdge.EdgeType) {
        case EFlowTilemapEdgeType::Wall:
            return FLinearColor(1, 0, 0.5f, 1.0f);
        case EFlowTilemapEdgeType::Door:
            return FLinearColor(0, 0, 1.0f, 1.0f);
        case EFlowTilemapEdgeType::Fence:
...
UTextureRenderTarget2D* FFlowTilemapRenderer::Create(UFlowTilemap* Tilemap, const FFlowTilemapRendererSettings& Settings) {
    if (!Tilemap) {
        return nullptr;
    }
    ...
    UTextureRenderTarget2D* Texture = FFlowCanvasHelper::CreateRenderTarget2D(
    ...
    FFlowCanvasHelper::BeginDrawCanvasToRenderTarget(Texture, Canvas, CanvasSize, RenderContext);
    ...
    if (!OneWayTileTexL || !OneWayTileTexR || !OneWayTileTexU || !OneWayTileTexD) {
        UE_LOG(LogFlowTileRenderer, Warning,
    ...
    for (int y = 0; y < Tilemap->GetHeight(); y++) {
        for (int x = 0; x < Tilemap->GetWidth(); x++) {
            ...
            bool bCanUseCustomColor = Cell.CellType != EFlowTilemapCellType::Door
            ...
            if (Cell.CellType == EFlowTilemapCellType::Custom) {
    ...
    for (int y = 0; y <= Tilemap->GetHeight(); y++) {
        for (int x = 0; x <= Tilemap->GetWidth(); x++) {
            ...
            {
                const FFlowTilemapEdge& EdgeH = Tilemap->GetEdgeH(x, y);
            ...
            {
                const FFlowTilemapEdge& EdgeV = Tilemap->GetEdgeV(x, y);
    ...
    for (int y = 0; y < Tilemap->GetHeight(); y++) {
        for (int x = 0; x < Tilemap->GetWidth(); x++) {
            ...
            if (Cell.CellType == EFlowTilemapCellType::Door && Tilemap->GetDoorMeta(FFlowTilemapCoord(Cell.TileCoord), DoorInfo)) {
    ...
    FFlowCanvasHelper::EndDrawCanvasToRenderTarget(Canvas, RenderContext);

    return Texture;
}

FLinearColor FFlowTilemapRenderer::GetCellColor(const FFlowTilemapCell& Cell) {
    switch (Cell.CellType) {
    case EFlowTilemapCellType::Empty:
        return FLinearColor::Transparent;

    case EFlowTilemapCellType::Floor:
        return FLinearColor::White;

    case EFlowTilemapCellType::Door:
        return FLinearColor::Blue;

    case EFlowTilemapCellType::Wall:
...
FLinearColor FFlowTilemapRenderer::GetSelectedCellColor(FLinearColor InColor) {
