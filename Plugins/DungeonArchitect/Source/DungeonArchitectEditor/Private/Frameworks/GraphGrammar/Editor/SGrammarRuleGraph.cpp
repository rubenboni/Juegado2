...
bool FGrammarRuleGraphHandler::CanDeleteNode(class UEdGraphNode* Node) {
    ...
    if (Node->IsA<UEdGraphNode_GrammarExecEntryNode>()) {
        return false;
    }
    return true;
}
...
void FProductionRuleGraphDropHandler::OnNodeTypeDrop(TSharedPtr<FDragDropOperation> Operation,
                                                     ...
                                                     TSharedPtr<SGraphEditor> GraphEditor,
                                                     TWeakObjectPtr<UEdGraph> Graph) {
    UEdGraph_GrammarExec* ExecutionGraph = Cast<UEdGraph_GrammarExec>(Graph.Get());
    ...
    if (DroppedRule && ExecutionGraph) {
        ...
        UEdGraphNode_GrammarExecRuleNode* RuleNode = ExecutionGraph->AddNewNode(DroppedRule);
        const float NodeOffset = 24;
        if (RuleNode) {
