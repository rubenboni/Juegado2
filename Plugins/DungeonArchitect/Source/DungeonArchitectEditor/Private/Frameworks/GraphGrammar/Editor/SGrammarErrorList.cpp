...
const FSlateBrush* GetLogIconName(FGrammarValidationEntryPtr ValidationEntry) {
    if (ValidationEntry->LogType == EGrammarLogType::Error) return FAppStyle::Get().GetBrush("Icons.Error");
    if (ValidationEntry->LogType == EGrammarLogType::Warning) return FAppStyle::Get().GetBrush("Icons.Warning");
