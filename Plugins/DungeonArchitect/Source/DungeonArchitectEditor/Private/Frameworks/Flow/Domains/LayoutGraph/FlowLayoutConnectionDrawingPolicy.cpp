...
#define LOCTEXT_NAMESPACE "FlowAbstractConnectionDrawingPolicy"

FFlowLayoutConnectionDrawingPolicy::FFlowLayoutConnectionDrawingPolicy(
...
void FFlowLayoutConnectionDrawingPolicy::DetermineWiringStyle(UEdGraphPin* OutputPin, UEdGraphPin* InputPin,
    ...
    if (OutputPin && InputPin) {
        UFlowLayoutEdGraphNode* SourceNode = Cast<UFlowLayoutEdGraphNode>(
            OutputPin->GetOwningNode());
        UFlowLayoutEdGraphNode* DestNode = Cast<UFlowLayoutEdGraphNode>(InputPin->GetOwningNode());
        if (SourceNode && DestNode) {
            UFlowLayoutEdGraph* AbstractEdGraph = Cast<UFlowLayoutEdGraph>(SourceNode->GetGraph());
            if (AbstractEdGraph && AbstractEdGraph->ScriptGraph.IsValid()) {
                UFlowAbstractGraphBase* ScriptGraph = AbstractEdGraph->ScriptGraph.Get();
...
void FFlowLayoutConnectionDrawingPolicy::Draw(TMap<TSharedRef<SWidget>, FArrangedWidget>& InPinGeometries,
    ...
    for (int32 NodeIndex = 0; NodeIndex < ArrangedNodes.Num(); ++NodeIndex) {
        ...
        TSharedRef<SGraphNode> ChildNode = StaticCastSharedRef<SGraphNode>(CurWidget.Widget);
        UEdGraphNode* NodeObj = ChildNode->GetNodeObj();
        if (NodeObj->IsA<UFlowLayoutEdGraphNode>()) {
            TSharedRef<SGraphNode_FlowLayoutNode> HostWidget = StaticCastSharedRef<SGraphNode_FlowLayoutNode>(CurWidget.Widget);
            TArray<TSharedPtr<SFlowItemOverlay>> NodeItemWidgets = HostWidget->GetNodeItemWidgets();
            ...
            TArray<SGraphNode_FlowLayoutNode::FLinkItemWidgetInfo> LinkItemWidgets = HostWidget->GetLinkItemWidgets();
            for (const SGraphNode_FlowLayoutNode::FLinkItemWidgetInfo& LinkItemInfo : LinkItemWidgets) {
    ...
    for (auto& Entry : ItemWidgetMap) {
        ...
        TSharedPtr<SFlowItemOverlay> ItemWidget = ItemInfo.ItemWidget.Pin();
...
void FFlowLayoutConnectionDrawingPolicy::DetermineLinkGeometry(FArrangedChildren& ArrangedNodes,
...
void FFlowLayoutConnectionDrawingPolicy::DrawPreviewConnector(const FGeometry& PinGeometry,
...
void FFlowLayoutConnectionDrawingPolicy::DrawSplineWithArrow(const FVector2D& StartAnchorPoint,
                                                                   const FVector2D& EndAnchorPoint,
                                                                   const FConnectionParams& Params) {
    Internal_DrawLineWithArrow(StartAnchorPoint, EndAnchorPoint, Params);
}

void FFlowLayoutConnectionDrawingPolicy::Internal_DrawLineWithArrow(
...
void FFlowLayoutConnectionDrawingPolicy::DrawSplineWithArrow(const FGeometry& StartGeom, const FGeometry& EndGeom,
                                                                   const FConnectionParams& Params) {
...
void FFlowLayoutConnectionDrawingPolicy::DrawConnection(int32 LayerId, const FVector2D& Start,
