...
void FDungeonEditorUtils::CreateDungeonItemFolder(ADungeon* Dungeon) {
    if (Dungeon && FActorFolders::IsAvailable()) {
        ...
        Folders.CreateFolder(World, FFolder(RootObject, Path));
...
void FDungeonEditorUtils::CollapseDungeonItemFolder(ADungeon* Dungeon) {
    if (Dungeon && Dungeon->GetWorld() && FActorFolders::IsAvailable()) {
        const FFolder::FRootObject& RootObject(Dungeon);
        if (FActorFolderProps* FolderProperties = FActorFolders::Get().GetFolderProperties(*Dungeon->GetWorld(), FFolder(RootObject, Dungeon->ItemFolderPath))) {
