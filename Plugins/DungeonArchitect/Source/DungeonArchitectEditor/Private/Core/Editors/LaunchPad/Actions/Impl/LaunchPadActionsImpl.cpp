...
namespace {
    ...
    bool ShowSaveDialog(const FString& InDefaultPath, FString& OutSavePath) {
        ...
        FSaveAssetDialogConfig SaveAssetDialogConfig;
        {
            ...
            SaveAssetDialogConfig.AssetClassNames.Add(TAssetClass::StaticClass()->GetClassPathName());
    ...
    template <typename TAssetClass>
    bool CloneAsset(TAssetClass* SourceAsset, const FString& SaveObjectPath, TAssetClass*& OutClonedAsset, FString& OutTargetPackageName) {
        OutTargetPackageName = FPackageName::ObjectPathToPackageName(SaveObjectPath);

        UEditorAssetSubsystem* EditorAssetSubsystem = GEditor->GetEditorSubsystem<UEditorAssetSubsystem>();
        UObject* DuplicateAsset = EditorAssetSubsystem->DuplicateLoadedAsset(SourceAsset, SaveObjectPath);
        OutClonedAsset = Cast<TAssetClass>(DuplicateAsset);
        if (OutClonedAsset) {
            EditorAssetSubsystem->SaveLoadedAsset(OutClonedAsset);
        }
...
void FLaunchPadActions::Exec_OpenFolder(const FString& InPath) {
    ...
    const FAssetData AssetData = AssetRegistry.GetAssetByObjectPath(FSoftObjectPath(*InPath));
