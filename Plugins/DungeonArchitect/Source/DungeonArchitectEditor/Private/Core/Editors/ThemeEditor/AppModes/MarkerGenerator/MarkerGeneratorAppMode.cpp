...
namespace MarkerGeneratorAppModeTabs {
	static const FName TabID_Patterns(TEXT("Patterns"));
	static const FName TabID_PatternViewport(TEXT("PatternViewport"));
	...
	DEFINE_THEME_EDITOR_TAB_FACTORY(Patterns,
				LOCTEXT("Caption_Patterns", "Patterns"),
				"LevelEditor.Tabs.Layers",
				LOCTEXT("TooltipMenu_Patterns", "Shows a list of defined patterns"),
				LOCTEXT("TooltipTab_Patterns", "Shows a list of defined patterns"))

	DEFINE_THEME_EDITOR_TAB_FACTORY(PatternViewport,
				LOCTEXT("Caption_PatternViewport", "Pattern Designer"),
				"LevelEditor.Tabs.Viewports",
				LOCTEXT("TooltipMenu_PatternViewport", "Design the pattern in the 3D viewport"),
				LOCTEXT("TooltipTab_PatternViewport", "Design the pattern in the 3D viewport"))
...
void FMarkerGeneratorAppMode::Init(TSubclassOf<UDungeonBuilder> InBuilderClass) {
	...
	PatternList = SNew(SEditableListView<UMarkerGenLayer*>)
	...
	TabFactories.RegisterFactory(MakeShareable(new MarkerGeneratorAppModeTabs::FThemeEditorTabFactory_Patterns(ThemeEditor, PatternList)));
	TabFactories.RegisterFactory(MakeShareable(new MarkerGeneratorAppModeTabs::FThemeEditorTabFactory_PatternViewport(ThemeEditor, PatternEditorHost)));
	...
	TabLayout = FTabManager::NewLayout(
			"MarkerGeneratorAppMode_Layout_v0.1.0")
		->AddArea
		(
			...
			(
				...
				(
					...
					(
						...
						->AddTab(MarkerGeneratorAppModeTabs::TabID_Patterns, ETabState::OpenedTab)
					)
					->Split
					(
						FTabManager::NewStack()
				...
				(
                	...
                	(
                		...
                		->AddTab(MarkerGeneratorAppModeTabs::TabID_PatternViewport, ETabState::OpenedTab)
                	)
                	->Split
                	(
                		FTabManager::NewStack()
...
FText FMarkerGeneratorAppMode::GetPatternEditorInvalidMessage() const {
	if (!AppModeImpl.IsValid()) {
		...
		return LOCTEXT("PatternEditorInvalidLayerLabel", "Please select a pattern to start editing");
	}
}
...
void FMarkerGeneratorAppMode::NotifyPostChange(const FPropertyChangedEvent& PropertyChangedEvent, FProperty* PropertyThatChanged) {
	...
	for (int32 Idx = 0; Idx < PropertyChangedEvent.GetNumObjectsBeingEdited(); Idx++) {
		...
		if (Obj->IsA<UMarkerGenPatternRule>()) {
			...
			else if (PropertyName == GET_MEMBER_NAME_CHECKED(UMarkerGenPatternRule, bHintWillInsertAssetHere)) {
				RebuildPreviewScene();
			}
		}
...
void FMarkerGeneratorAppMode::OnLayerAdd() const {
	static const FText DefaultLayerName = LOCTEXT("DefaultLayerName", "Pattern");
	...
	if (UMarkerGenModel* Model = GetMarkerGenerationModel()) {
		...
		if (NewLayer) {
			...
			Model->Modify();
			PatternList->RefreshListView();
			PatternList->SetItemSelection(NewLayer);
		}
	}
}
