...
void FSnapGridFlowEditor::InitDungeonConfig(UDungeonConfig* Config) {
	if (USnapGridFlowConfig* SgfConfig = Cast<USnapGridFlowConfig>(Config)) {
		SgfConfig->ParameterOverrides = EditorSettings->ParameterOverrides;
	}
}

void FSnapGridFlowEditor::CreateDomainEditorsImpl() {
	// Register the domain editors
	DomainEditors.Add(MakeShareable(new FSnapGridFlowDomainEdLayoutGraph3D));
}
...
ADungeon* FSnapGridFlowEditor::CreatePreviewDungeon(UWorld* World) {
	...
	if (PreviewActor) {
		...
		USnapGridFlowConfig* SGFConfig = Cast<USnapGridFlowConfig>(PreviewActor->GetConfig());
		if (SGFConfig) {
			// Setup the grid flow graph asset to the currently edited asset
			SGFConfig->FlowGraph = Cast<USnapGridFlowAsset>(AssetBeingEdited);
		}
	}
	return PreviewActor;
}
...
void FSnapGridFlowEditor::UpgradeAsset() const {
	...
	if (SGFAsset->Version + 1 == static_cast<int>(ESnapGridFlowAssetVersion::DeprecateTaskExtensions)) {
		for (UEdGraphNode* EdNode : SGFAsset->ExecEdGraph->Nodes) {
			if (const UFlowExecEdGraphNode_Task* TaskNode = Cast<UFlowExecEdGraphNode_Task>(EdNode)) {
...
TSharedPtr<FTabManager::FLayout> FSnapGridFlowEditor::CreateFrameLayout() const {
	TSharedPtr<FTabManager::FLayout> Layout = FTabManager::NewLayout(ConstructLayoutName("SnapFlowEditor_v1.0.0"))
		->AddArea
		(
